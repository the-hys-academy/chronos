...
public sealed class PqsqlDB : IRegionRepository {
    ...
    public PqsqlDB(string? connStr) { 

        connStr = connStr ?? "Host=localhost;Username=hys;Password=123;Database=mydatabase";
        }
    public async Task<int?> Create(Region reg, Error err, CancellationToken ct = default){
        ...
        await using (var cmd = 
            new NpgsqlCommand(
            "INSERT INTO Region(name, region) VALUES (@name, @region) RETURNING Id", 
            conn)) {
